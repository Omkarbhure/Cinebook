...
// Atomic conditional update — prevents race conditions where two concurrent
// requests both pass a balance check and drive the balance negative.
exports.deductWallet = async (userId, amount, description) => {
  const updated = await User.findOneAndUpdate(
    { _id: userId, walletBalance: { $gte: amount } },
    {
      $inc: { walletBalance: -amount },
      $push: { walletTransactions: { type: 'debit', amount, description, date: new Date() } },
    },
    { new: true }
  );
