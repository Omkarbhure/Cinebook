...
exports.createBooking = async (req, res) => {
  try {
    ...
    // ── Step 3: Charge wallet BEFORE creating any records ─
    // Deduct first so that if anything downstream fails the money was never
    // committed. If seat assignment later fails we refund in the rollback path.
    // For non-wallet payments this step is a no-op.
    if (paymentMethod === 'cinebook_wallet') {
      try {
        await deductWallet(
          req.user._id,
          grandTotal,
          'Booking payment — ' + (show.movie.title || 'Movie')
        );
      } catch (walletErr) {
        return res.status(400).json({
          success: false,
          message: walletErr.message === 'Insufficient wallet balance'
            ? 'Insufficient CineBook Wallet balance. Please top up and try again.'
    ...
    if (!allUpdated) {
      // Rollback: delete booking, release any partially-marked seats, refund wallet
      try {
        ...
        // Refund wallet if we already charged it
        if (paymentMethod === 'cinebook_wallet') {
          await refundWallet(req.user._id, grandTotal, 'Refund — seat conflict during booking');
        }
      } catch (rollbackErr) {
    ...
    // ── Step 6: Update movie booking count ────────────────
...
exports.cancelBooking = async (req, res) => {
  try {
    ...
    if (booking.status === 'used')
      return res.status(400).json({ success: false, message: 'This ticket has already been scanned and used. It cannot be cancelled.' });
...
// ─── Verify Booking via QR scan (admin/staff only) ───────
// Lookup is by verifyToken — a 32-byte random hex string embedded in the QR
// code, separate from the human-readable bookingId. This prevents low-entropy
// guessing attacks that could flip a customer's ticket to 'used' before arrival.
exports.verifyBooking = async (req, res) => {
  try {
    const booking = await Booking.findOne({ verifyToken: req.params.verifyToken })
      .populate({ path: 'show', populate: { path: 'theater' } })
      .populate('movie user');
