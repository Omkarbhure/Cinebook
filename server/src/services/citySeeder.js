...
const seedAllCities = async () => {
  try {
    ...
    // Fix #3: Only delete PAST shows that have NO booked seats.
    // Using seats: { $not: { $elemMatch: { userId: { $ne: null } } } } is correct —
    // 'seats.userId': null alone would match shows where *any* seat is null (almost all),
    // causing shows with real paid bookings to be deleted.
