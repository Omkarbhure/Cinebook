  ...
  useEffect(() => {
    const fetchMovies = async () => {
      ...
      try {
        ...
        let movieList: Movie[] = res?.data?.movies || [];

        // If city filter returned zero results, fall back to all cities.
        // This happens when the scheduler hasn't generated shows for the
        // selected city yet (e.g. first run, or shows were cleared).
        if (movieList.length === 0 && city) {
          const fallbackQuery = { ...query };
          delete fallbackQuery.city;
          const fallbackRes = await getMovies(fallbackQuery);
          movieList = fallbackRes?.data?.movies || [];
          setTotal(fallbackRes?.data?.total || movieList.length);
        } else {
          setTotal(res?.data?.total || movieList.length);
        }
