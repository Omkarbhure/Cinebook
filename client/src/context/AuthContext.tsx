...
interface AuthContextType extends AuthState {
  ...
  resendLoginOtp: (userId: string, email?: string) => Promise<{ maskedEmail?: string; userId?: string }>;
  ...
  resendRegisterOtp: (userId: string, email?: string) => Promise<{ maskedEmail?: string; userId?: string }>;
...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const resendLoginOtp = async (userId: string, email?: string): Promise<{ maskedEmail?: string; userId?: string }> => {
    const res = await axios.post(`${API}/auth/resend-login-otp`, { userId, email });
    return { maskedEmail: res.data.maskedEmail, userId: res.data.userId };
  };
  ...
  const register = async (name: string, email: string, password: string, phone?: string): Promise<{ requiresOtp: boolean; maskedEmail: string; userId: string }> => {
    const payload: { name: string; email: string; password: string; phone?: string } = { name, email, password };
  ...
  const resendRegisterOtp = async (userId: string, email?: string): Promise<{ maskedEmail?: string; userId?: string }> => {
    const res = await axios.post(`${API}/auth/resend-register-otp`, { userId, email });
    return { maskedEmail: res.data.maskedEmail, userId: res.data.userId };
  };
